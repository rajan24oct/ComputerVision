...
from com.ml.objects.Detection.keras_retinanet.models.resnet import resnet50_retinanet
from com.ml.objects.Detection.keras_retinanet.utils.image import read_image_bgr, read_image_array, read_image_stream, \
    preprocess_image, resize_image
from com.ml.objects.Detection.keras_retinanet.utils.visualization import draw_box, draw_caption
from com.ml.objects.Detection.keras_retinanet.utils.colors import label_color
...
from com.ml.objects.Detection.YOLOv3.models import yolo_main, tiny_yolo_main
from com.ml.objects.Detection.YOLOv3.utils import letterbox_image, yolo_eval
...
class ObjectDetection:
    ...
    def loadModel(self, detection_speed="normal"):
        """
                ...
                The value is used to reduce the time it takes to detect com.ml.objects in an image, down to about a 10% of the normal time, with
                 with just slight reduction in the number of com.ml.objects detected.
    ...
    def detectObjectsFromImage(self, input_image="", output_image_path="", input_type="file", output_type="file",
        ...
        """
            'detectObjectsFromImage()' function is used to detect com.ml.objects observable in the given image path:
                    ...
                    * extract_detected_objects (optional) , option to save each object detected individually as an image and return an array of the com.ml.objects' image path.
            ...
            - If extract_detected_objects = False or at its default value and output_type = 'file' or
                ...
                1. an array of dictionaries, with each dictionary corresponding to the com.ml.objects
            ...
            - If extract_detected_objects = False or at its default value and output_type = 'array' ,
              Then the function will return:
                ...
                2. an array of dictionaries, with each dictionary corresponding to the com.ml.objects
                    detected in the image. Each dictionary contains the following property:
                    * name (string)
                    * percentage_probability (float)
                    * box_points (tuple of x1,y1,x2 and y2 coordinates)
            ...
            try:
                if (self.__modelType == "retinanet"):
                    ...
                    for index, (label, score), in enumerate(zip(predicted_numbers, scores)):
                        ...
                        objects_dir = output_image_path + "-com.ml.objects"
    ...
    def CustomObjects(self, person=False, bicycle=False, car=False, motorcycle=False, airplane=False,
        ...
        """
                         The 'CustomObjects()' function allows you to handpick the type of com.ml.objects you want to detect
                         from an image. The com.ml.objects are pre-initiated in the function variables and predefined as 'False',
                         which you can easily set to true for any number of com.ml.objects available.  This function
                         returns a dictionary which must be parsed into the 'detectCustomObjectsFromImage()'. Detecting
                          custom com.ml.objects only happens when you call the function 'detectCustomObjectsFromImage()'
    ...
    def detectCustomObjectsFromImage(self, custom_objects=None, input_image="", output_image_path="", input_type="file",
        ...
        """
                    'detectCustomObjectsFromImage()' function is used to detect predefined com.ml.objects observable in the given image path:
                            * custom_objects , an instance of the CustomObject class to filter which com.ml.objects to detect
                            * input_image , which can be file to path, image numpy array or image file stream
                            * output_image_path , file path to the output image that will contain the detection boxes and label, if output_type="file"
                            * input_type (optional) , file path/numpy array/image file stream of the image. Acceptable values are "file", "array" and "stream"
                            * output_type (optional) , file path/numpy array/image file stream of the image. Acceptable values are "file" and "array"
                            * extract_detected_objects (optional, False by default) , option to save each object detected individually as an image and return an array of the com.ml.objects' image path.
                            * minimum_percentage_probability (optional, 50 by default) , option to set the minimum percentage probability for nominating a detected object for output.
                            * display_percentage_probability (optional, True by default), option to show or hide the percentage probability of each object in the saved/returned detected image
                            * display_display_object_name (optional, True by default), option to show or hide the name of each object in the saved/returned detected image
...
class VideoObjectDetection:
    ...
    def detectObjectsFromVideo(self, input_file_path="", camera_input=None, output_file_path="", frames_per_second=20,
        ...
        """
                    'detectObjectsFromVideo()' function is used to detect com.ml.objects observable in the given video path or a camera input:
                            ...
                            * per_frame_function (optional), this parameter allows you to parse in a function you will want to execute after
                                                                ...
                                                                -- a dictionary with with keys being the name of each unique com.ml.objects and value
                            ...
                            * per_second_function (optional), this parameter allows you to parse in a function you will want to execute after
                                                                ...
                                                                -- an array of dictionaries, with each dictionary corresponding to each frame in the past second, and the keys of each dictionary are the name of the number of unique com.ml.objects detected in each frame, and the key values are the number of instances of the com.ml.objects found in the frame
                            ...
                            * per_minute_function (optional), this parameter allows you to parse in a function you will want to execute after
                                                                ...
                                                                -- an array of dictionaries, with each dictionary corresponding to each frame in the past minute, and the keys of each dictionary are the name of the number of unique com.ml.objects detected in each frame, and the key values are the number of instances of the com.ml.objects found in the frame
                            ...
                            * video_complete_function (optional), this parameter allows you to parse in a function you will want to execute after
                                                                ...
                                                                -- an array of dictionaries, with each dictionary corresponding to each frame in the entire video, and the keys of each dictionary are the name of the number of unique com.ml.objects detected in each frame, and the key values are the number of instances of the com.ml.objects found in the frame
    ...
    def CustomObjects(self, person=False, bicycle=False, car=False, motorcycle=False, airplane=False,
        ...
        """
                         The 'CustomObjects()' function allows you to handpick the type of com.ml.objects you want to detect
                         from a video. The com.ml.objects are pre-initiated in the function variables and predefined as 'False',
                         which you can easily set to true for any number of com.ml.objects available.  This function
                         returns a dictionary which must be parsed into the 'detectCustomObjectsFromVideo()'. Detecting
                          custom com.ml.objects only happens when you call the function 'detectCustomObjectsFromVideo()'
        ...
        """
                            'detectObjectsFromVideo()' function is used to detect specific object(s) observable in the given video path or given camera live stream input:
                                    ...
                                    * per_frame_function (optional), this parameter allows you to parse in a function you will want to execute after
                                                                        ...
                                                                        -- an array of dictinaries, with each dictinary corresponding to each object detected.
                                                                            Each dictionary contains 'name', 'percentage_probability' and 'box_points'
                                                                        -- a dictionay with with keys being the name of each unique com.ml.objects and value
                                                                            are the number of instances of the object present
                                                                        -- If return_detected_frame is set to True, the numpy array of the detected frame will be parsed
                                                                            as the fourth value into the function
